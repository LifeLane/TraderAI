...
async function submitUserMessage(content: string) {
  export async function submitUserMessage(content: string) {
    ...
    if (match) {
      ...
      return {
        id: nanoid(),
        display: await submitTradeAnalysis(symbol)
      }
    }
            ...
            return (
              <BotCard>
                <StockChart 
                  ...
                  prices={[174.2, 173.5, 172.8, 171.9, 172.3, 173.0, 172.5, 171.0, 170.5, 171.2, 170.1, 170.9, 171.5, 172.4]} 
                />
                <TradeIdeaButton 
                  symbol={symbol} 
                  prices={[174.2, 173.5, 172.8, 171.9, 172.3, 173.0, 172.5, 171.0, 170.5, 171.2, 170.1, 170.9, 171.5, 172.4]} 
...
export async function submitTradeAnalysis(symbol: string) {
  ...
  try {
    if (symbol.endsWith('USD')) {
      // Cryptocurrency flow
      prices = await fetchCryptoPrices(symbol)
    } else {
      // Stock flow
      prices = await fetchHistoricalPrices(symbol)
    }
